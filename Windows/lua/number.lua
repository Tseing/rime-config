...
local function num2suzhou(num)
	...
	local horizontal_suzhou = {"一","二","三"}
	local one_two_three = {table.unpack(suzhou, 2, 4)}
	local result = "" 
	...
	for pos=1, string.len(num) do
		...
		if pos > 1 then
			if digit > 0 and digit < 4 then
				if contains(one_two_three, string.sub(result, -3)) then
					result = result .. horizontal_suzhou[digit]
					goto continue
				end
			end
		end
...
--		table.insert(result,{number2cnChar(numberPart.int,1,{"萬", "億"},{"〇","一","十","点"})..number2zh(numberPart.dec,1),"〔大写〕"})
	...
	table.insert(result, {num2suzhou(numberPart.int) .. numberPart.dot .. num2suzhou(numberPart.dec), "〔蘇州碼〕"})
	return result
end
